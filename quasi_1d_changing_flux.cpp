...
#define lat_len 100//lattice length
#define isRing 0//1 if the model is a ring, 0 otherwise
#define isMagnet 0//1 if there is a magnetic field, 0 otherwise
#define fluxDiv 200// number of flux resolution
#define lambda 0//potential amplitude
...
//#define SOC_amp 1.0
#define FluxCase 5
#define ImpureAmp -0.1//amplitude of impurity
#define IterLim 3000
#define isCurCalc 1
#define isCurPrint 1
...
#define isEEcalc 1
#define isSymmetric 1//relevant only for impurity(FluxCase==5). defines whether the impurity is symmetric or not
//#define isMixed 0//take mixed states as trial wave functions
...
char FileStrE[10] = "Energy.txt";//_impurity01_symetry
char FileStrCurLon[11] = "Cur_lon.txt";
char FileStrCurTrn[11] = "Cur_trn.txt";
char FileStrCurSum[11] = "Cur_sum.txt";
char FileStrCur[12] = "Cur_flip.txt";
char FileStrState[18] = "EE_half_system.txt";
...
void EE_calc(double* stateI, double* stateR,int interest_len,Engine *ep,int dif) {
	...
	mxArray *mxCorrI = mxCreateNumericMatrix(MatSize, MatSize,mxDOUBLE_CLASS,mxREAL);
	mxArray *mxCorrR = mxCreateNumericMatrix(MatSize, MatSize, mxDOUBLE_CLASS, mxREAL);
	for (i = 0; i < MatSize; i++) {
		for (j = 0; j < MatSize; j++) {
...
void CurCalcV4(int fluxInd, double* CurLong[], double* CurTran[], int interest_len, Engine *ep,int CaseInd) {//another version of calculating the current
	...
	if (FluxCase != 6) {
		...
		if (isRing) {
			engEvalString(ep, "temp2=temp(1,1:MyLen).*conj(temp(end-1,1:MyLen))*exp(-1i*fluxPh);");
			engEvalString(ep, "tempLonUp= tempLonUp(:,fluxVal+1) + sum(temp2+SumFactor*conj(temp2),2);");
			engEvalString(ep, "temp2=temp(2,1:MyLen).*conj(temp(end,1:MyLen))*exp(-1i*fluxPh);");
...
void RingCase(double* HR0[], double* HI0[]) {
	//if (isFlip == 0 || t_flip==0.0)
	fluxVal = fluxVal / lat_len;
	HR0[0][2 * lat_len - 2] = t_amp*cos(f1*(fluxVal + alpha * 2 * M_PI));
	HI0[0][2 * lat_len - 2] = -t_amp*sin(f1*(fluxVal + alpha * 2 * M_PI));
	HR0[2 * lat_len - 2][0] = t_amp*cos(f1*(fluxVal + alpha * 2 * M_PI));
	HI0[2 * lat_len - 2][0] = t_amp*sin(f1*(fluxVal + alpha * 2 * M_PI));
	HR0[1][2 * lat_len - 1] = t_amp*cos(f2*(fluxVal + alpha * 2 * M_PI));// +2 * M_PI / fluxDiv);
	HI0[1][2 * lat_len - 1] = -t_amp*sin(f2*(fluxVal + alpha * 2 * M_PI));// +2 * M_PI / fluxDiv);
	HR0[2 * lat_len - 1][1] = t_amp*cos(f2*(fluxVal + alpha * 2 * M_PI));// + 2 * M_PI / fluxDiv);
	HI0[2 * lat_len - 1][1] = t_amp*sin(f2*(fluxVal + alpha * 2 * M_PI));// +2 * M_PI / fluxDiv);
}

void CreateSingleChain(double sigma, double* HR0[]) {//creates the hamiltonian of a single chain
	int i, ind1, dif, low_lim;
	int MatSize = 2 * lat_len;

	HR0[0][0]= lambda*cos(phi);
	HR0[0][1] = t_amp;
	for (i = 1; i < lat_len - 1; i++)//create the matrix
	{
		HR0[i][i] = lambda*cos(2.0*M_PI*sigma*i + phi);
		HR0[i][i+1] = t_amp;
		HR0[i][i - 1] = t_amp;
	}
	HR0[lat_len-1][lat_len-1] = lambda*cos(2.0*M_PI*sigma*(lat_len-1) + phi);
	HR0[lat_len-1][lat_len - 2] = t_amp;

}

void createMH(double sigma, double* HR0[], double* HI0[])
{
	int i;
	int MatSize = 2 * lat_len;
...
void changing_flux(double* HR[], double* HI[], double* HR0[], double* HI0[], double** finalE, double* CurLon[], double* CurTrn[], int low_lim, int high_lim, int MaxFlux, int cmpnum, double sigma, Engine *ep,FILE* fpLog) {
	...
	for (fluxInd = 0; fluxInd < MaxFlux; fluxInd++) {
		...
		mexHigh = mxCreateDoubleScalar((double)high_lim);
		engPutVariable(ep, "high_lim", mexHigh);// this is the highest filling
		mexLow = mxCreateDoubleScalar((double)low_lim);
		...
		for (interest_len = low_lim; interest_len <= high_lim; interest_len++)//=len_dif)
		{
			cmpnum = interest_len;
			if (fluxInd % 10 == 0) {
				system("cls");
				printf("%d,%d\n", interest_len, fluxInd);
			}
			...
			if (isHubbard || isFock || isHartr) {
				...
				while ((dif > converg_lim && count < IterLim) && count_dif < 25)
				{
					...
					/*for (i = 0; i < lat_len; i++) {
						matmul = MatMulReal(stateR, stateI, stateR, stateI, i, i, lat_len, 'T');//T for  transposing the second matrix, N for normal case
						if (matmul!= 1)
							printf("states wrong, value is:%.12f\n", matmul);
					}*/
					/*if (count_dif >= 10 && isMixed == 0) {
						isMixed = 1;
						count = 0;
						count_dif = 0;
					}*/
...
void changing_SOC(double* HR[], double* HI[], double* HR0[], double* HI0[], double* finalE[], double* CurLon[], double* CurTrn[], int low_lim, int high_lim, int cmpnum, double sigma, Engine *ep, FILE* fpLog) {
	...
	for (SOCInd = 0; SOCInd < SOCDiv; SOCInd++) {
		...
		MakeInitState(&stateR, &stateI, ep, LengthFactor);
		//if (isFock || isHartr) {
		for (interest_len = low_lim; interest_len <= high_lim; interest_len++)
		{
			if (SOCInd % 10 == 0) {
				printf("%d,%d\n", interest_len, SOCInd);
			}
...
void printCurSum(Engine *ep,FILE *fpCurSum,int high_lim,int low_lim) {
	int len = high_lim - low_lim + 1;
...
void PrintLog(int low_lim,int high_lim,FILE* fpLog) {
	...
	if (FluxCase == 5) {
		...
		fprintf(fpLog, "Impurity Symmetric:%d\n",isSymmetric);
	}
	else
...
int main()
{
	int high_lim, low_lim, i, j, cmpnum = lat_len, MatSize = 2 * lat_len, MaxFlux,rel_max;
	double sigma = sqrt(30.0);//modualtion;
										 //    double complex hemilton[lat_len][lat_len];
										 //	double r_H[lat_len][lat_len];//the real part of the hamiltonian
										 //    double i_H[lat_len][lat_len];//the imaginary part of the hamiltonian
	...
	//double **CurTrn = (double**)malloc(2 * lat_len * sizeof(double*));
	double **CurTrn = (double**)malloc(lat_len * sizeof(double*));
	//double flux[fluxDiv];															 //   for (i=0;i<fluxDiv;i++)
	//       flux[i]=0.0;
	//char buffer[BSZ  + 1];
	...
	fpLog = _fsopen(str1, "wt", OF_SHARE_DENY_NONE);
	//init_array(HI0, MatSize, MatSize);
	...
	mxArray *mexStrE = mxCreateString(FileStrE);
	engPutVariable(ep, "StrE", mexStrE);
	//engEvalString(ep, "path='C:\Users\Arik\Documents\Visual Studio 2015\Projects\quasi_1d_changing_flux\';temp=strsplit(StrE,'.');StrE=strcat(path,temp{1},'.mat')");
	engEvalString(ep, "temp=strsplit(StrE,'.');StrE=strcat(temp{1},'.mat')");
	mxArray *mexStrCur = mxCreateString(FileStrCur);
	engPutVariable(ep, "StrCur", mexStrCur);
	//engEvalString(ep, "path='C:\Users\Arik\Documents\Visual Studio 2015\Projects\quasi_1d_changing_flux\';temp=strsplit(StrCur,'.');StrCur=strcat(path,temp{1},'.mat')");
	engEvalString(ep, "temp=strsplit(StrCur,'.');StrCur=strcat(temp{1},'.mat')");
	mxArray *mexStrState = mxCreateString(FileStrState);
	engPutVariable(ep, "StrState", mexStrState);
	engEvalString(ep, "temp=strsplit(StrState,'.');StrState=strcat(temp{1},'.mat')");
	...
	//init_array(CurTrn, MatSize, rel_max);
	init_array(CurTrn, lat_len, rel_max);
	init_array(CurLon, MatSize, rel_max);
	if ((isHartr || isHubbard || isFock) && isMagnet==1) {
		low_lim = 80;//192;// (int)4 * lat_len / 10; // (int)1;//0.8*lat_len;
		high_lim = 80;//192;// (int)2 * lat_len;// 1.2*lat_len;//2*lat_len;
	}
	else {
		low_lim = 55;
		high_lim = 55;
	}
	PrintLog(low_lim, high_lim, fpLog);
	//low_lim = lat_len; 
	//high_lim = lat_len;
	if (isMagnet)
	...
	if (isConstSOC == 1 || isSOC==0) {
		SOC_amp = 0.5;
		changing_flux(HR, HI, HR0, HI0, finalE, CurLon,CurTrn, low_lim, high_lim, MaxFlux, cmpnum, sigma, ep,fpLog);
	}
  	else {
		fluxVal = 0.0;
		changing_SOC(HR, HI, HR0, HI0, finalE, CurLon, CurTrn, low_lim, high_lim, cmpnum, sigma,ep, fpLog);
	}

	//MyPrints(rel_max,finalE,CurLon,CurTrn);

	if (isCurSumCalc) {
		printCurSum(ep, fpCurSum,high_lim,low_lim);
		fclose(fpCurSum);
	}
